...
export default function ShoesTable({ rows }: Props) {
    ...
    function Th({ label, k }: { label: string; k: SortKey }) {
        ...
        return (
            <th
                className="px-3 py-2.5 text-left font-semibold border-b border-gray-300 cursor-pointer select-none"
    ...
    return (
        <div className="overflow-x-auto rounded-lg border relative">
            <table className="min-w-[1040px] w-full text-[15px] leading-6 text-gray-900">
                    ...
                    {sorted.map((r) => {
                        ...
                        return (
                            <tr key={r.id} className="odd:bg-white even:bg-gray-50">
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.text(r.brand)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.text(r.model)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.text(r.version)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.year(r.year)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200 capitalize">{fmt.text(r.terrain)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200 capitalize">{fmt.text(r.category)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.g(r.weight_men)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.g(r.weight_women)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.mm(r.heel)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.mm(r.forefoot)}</td>
                                <td className="px-3 py-2.5 border-b border-gray-200">{fmt.mm(r.drop)}</td>
